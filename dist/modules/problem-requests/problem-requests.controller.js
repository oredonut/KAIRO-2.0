    ...
    static async analyzeInput(req, res) {
        try {
            ...
            if (!input || typeof input !== 'string') {
                return (0, response_1.sendError)(res, 'Input natural language problem string is required.');
            }
            // Step 1: Run AI analysis pipeline with Zod schema validation & retries (Requirement #16)
            const analysisResult = await ai_service_1.AIService.analyzeProblem(input);
            ...
            await firebase_1.db.collection('problem_requests').doc(problemRequestId).set(problemRequest);
            ...
            await firebase_1.db.collection('problem_analyses').doc(analysisId).set(problemAnalysis);
            ...
            return (0, response_1.sendError)(res, err.message || 'Failed to analyze problem request.', 500);
        }
    }
    static async getById(req, res) {
        try {
            const { id } = req.params;
            const requestDoc = await firebase_1.db.collection('problem_requests').doc(id).get();
            if (!requestDoc.exists) {
                return (0, response_1.sendError)(res, 'Problem request not found.', 404);
            }
            const problemRequest = requestDoc.data();
            const analysisDoc = await firebase_1.db.collection('problem_analyses').doc('pa_' + id).get();
            return (0, response_1.sendSuccess)(res, {
                ...problemRequest,
            ...
            return (0, response_1.sendError)(res, err.message || 'Failed to fetch problem request.', 500);
        }
    }
    static async getMyRequests(req, res) {
        try {
            const customerId = req.user?.id;
            const snapshot = await firebase_1.db.collection('problem_requests').where('customerId', '==', customerId).get();
            const requests = snapshot.docs.map((doc) => doc.data());
            return (0, response_1.sendSuccess)(res, requests, 'User problem requests retrieved.');
        }
        catch (err) {
            return (0, response_1.sendError)(res, err.message || 'Failed to fetch requests.', 500);
        }
    }
}
