...
class SubscriptionsController {
    static async subscribePro(req, res) {
        try {
            ...
            await firebase_1.db.collection('subscriptions').doc(subId).set(subscription);
            await firebase_1.db.collection('professional_profiles').doc(profId).update({ isPro: true });
            return (0, response_1.sendSuccess)(res, subscription, 'Professional subscription activated to PRO tier.', 201);
        }
        catch (err) {
            return (0, response_1.sendError)(res, err.message || 'Subscription failed.', 500);
        }
    }
    static async getMySubscription(req, res) {
        try {
            ...
            const profId = profQuery.docs[0].id;
            const snapshot = await firebase_1.db.collection('subscriptions').where('professionalId', '==', profId).get();
            const subs = snapshot.docs.map((doc) => doc.data());
            return (0, response_1.sendSuccess)(res, subs[0] || null, 'Subscription details retrieved.');
        }
        catch (err) {
